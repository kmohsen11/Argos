...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { firstName, lastName, email, productType, size, deviceType } = req.body as PreorderData;
  ...
  try {
    ...
    return res.status(500).json({ error: 'mail_error', message: err instanceof Error ? err.message : 'Unknown error' });
  }
} 